...
function lowestPublicRecord(slug) {
  const rows = eligible.filter((p) => p.slug === slug);
...
const recommendations = recommendationSlugs
  .map((slug) => lowestPublicRecord(slug))
  .filter(Boolean)
  .map((p) => ({
    slug: p.slug,
    href: canonicalPath(p.slug),
    name: p.name,
    brand: p.brand,
    category: p.category,
    tier: p.tier ?? null,
...
writeFileSync(
  ...
  `// Generated by scripts/generate-homepage-v2-view.mjs. Do not hand-edit build output.\nimport type { PublicHomepageView } from "@/lib/homepageV2";\n\nexport const HOMEPAGE_V2 = ${JSON.stringify(output, null, 2)} as const satisfies PublicHomepageView;\n`,
  "utf8",
);
console.log(`[homepage-v2] ${uniqueSlugs.length} product families, ${recommendations.length} recommendations, mode=${output.publication.mode}`);

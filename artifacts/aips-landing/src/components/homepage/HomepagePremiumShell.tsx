...
const FOOTER_PRODUCT_LINKS = [
  { label: "ChatGPT", href: "/chatgpt-plans-bangladesh" },
  ...
  { label: "GitHub Copilot", href: "/github-copilot-bangladesh" },
  { label: "All products", href: "/products" },
] as const;

function isActiveProduct(product: LiteProduct) {
...
function NavButton({ id, label, openMenu, setOpenMenu }: {
  id: MenuKey;
  label: string;
        ...
        {SOLUTIONS.map((solution) => {
          ...
          return (
            <a key={solution.href} href={solution.href} className="group rounded-2xl border border-white/10 bg-white/[0.025] p-4 transition hover:-translate-y-0.5 hover:border-[#f4b942]/30 hover:bg-[#f4b942]/[0.045]">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-300 group-hover:text-[#f4b942]"><Icon className="h-4 w-4" /></div>
          ...
          {query.trim().length < 2 ? (
            ...
            results.map((product) => (
              <a key={product.slug} href={productPath(product.slug)} className="flex items-center gap-4 border-b border-white/8 px-5 py-3.5 last:border-0 hover:bg-white/[0.04]">
                <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-white">{product.name}</p><p className="mt-0.5 text-xs text-slate-500">{product.brand} · {product.category}</p></div>
  ...
  return (
    <>
      ...
      <header data-testid="premium-homepage-nav" onMouseLeave={scheduleClose} className="sticky top-0 z-[70] border-b border-white/10 bg-[#050912]/88 backdrop-blur-2xl supports-[backdrop-filter]:bg-[#050912]/76">
        ...
        <AnimatePresence>
          {openMenu && (
            <motion.div initial={reduced ? false : { opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -8 }} transition={{ duration: 0.18, ease: "easeOut" }} className="absolute left-0 right-0 top-full hidden border-b border-white/10 bg-[#050912]/98 shadow-[0_30px_80px_rgba(0,0,0,.45)] backdrop-blur-2xl lg:block">
      ...
      <section aria-labelledby="browse-categories-title" className="border-t border-white/10 px-4 py-16 md:px-8 lg:py-20">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#f4b942]">Explore by category</p><h2 id="browse-categories-title" className="mt-3 text-3xl font-semibold tracking-[-0.03em] sm:text-4xl">Every major AI category, one clear path into the catalog.</h2><p className="mt-4 text-base leading-7 text-slate-400">These are real category destinations with direct internal links to the tools inside them—not decorative filters that disappear for crawlers.</p></div>
            <a href="/products" className="inline-flex items-center gap-2 text-sm font-bold text-[#f4b942]">All {ACTIVE_PRODUCT_FAMILIES} tool families <ArrowRight className="h-4 w-4" /></a>
          </div>
      ...
      <section className="px-4 py-16 md:px-8 lg:py-20">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#f4b942]">How to choose</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.03em] sm:text-4xl">A simple path from discovery to the right product page.</h2><p className="mt-4 text-base leading-7 text-slate-400">The homepage can be cinematic without hiding the information needed for a buying decision.</p></div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[["01", "Choose the job", "Start with a category, goal, budget or comparison instead of guessing a brand."], ["02", "Check the exact plan", "Read the product page for the current access model, public price and governed details."], ["03", "Use the buying path", "Continue through the current order or support route only after the plan fits your needs."]].map(([number, title, copy]) => (
                <motion.div key={number} whileHover={reduced ? undefined : { y: -4 }} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5"><p className="text-xs font-extrabold tracking-[0.16em] text-[#f4b942]">{number}</p><h3 className="mt-3 font-bold text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{copy}</p></motion.div>
              ))}
            </div>
          </div>
          <div className="mt-10 rounded-3xl border border-white/10 bg-[linear-gradient(135deg,rgba(244,185,66,.06),rgba(255,255,255,.02))] p-5 sm:p-7">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#f4b942]">Deeper discovery</p><h2 className="mt-2 text-2xl font-bold">Budget, comparison and guide pages should be one click away.</h2><p className="mt-2 text-sm leading-6 text-slate-400">These links help visitors self-select and make the site hierarchy explicit.</p></div><a href="/guides" className="inline-flex items-center gap-2 text-sm font-bold text-[#f4b942]">Explore all guides <ArrowRight className="h-4 w-4" /></a></div>
            <div className="mt-5 flex flex-wrap gap-2">{SEO_DEEP_LINKS.map((link) => <a key={link.href} href={link.href} className="rounded-full border border-white/10 bg-white/[0.025] px-3.5 py-2 text-sm font-semibold text-slate-300 transition hover:border-[#f4b942]/30 hover:text-white">{link.label}</a>)}</div>
          </div>
        </div>
      </section>
    </div>
  ...
  return (
    <footer className="border-t border-white/10 bg-[#03060b] text-white">
      <div className="mx-auto max-w-7xl px-4 py-12 md:px-8">
        ...
        <div className="mt-9 flex flex-col gap-3 border-t border-white/8 pt-5 text-xs text-slate-600 sm:flex-row sm:items-center sm:justify-between"><p>© 2026 AI Premium Shop</p><nav aria-label="Legal" className="flex flex-wrap gap-4"><a href="/privacy-policy" className="hover:text-white">Privacy</a><a href="/terms" className="hover:text-white">Terms</a><a href="/refund-policy" className="hover:text-white">Refund policy</a><a href="/contact" className="hover:text-white">Contact</a></nav></div>
      </div>
    </footer>
  );
}

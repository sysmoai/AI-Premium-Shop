...
| 35 | SEO content-gap opportunities found by real search-intent research (2026-08-07, `docs/agent/RESEARCH-CACHE.md`'s market-research section), distinct from `docs/seo/GAP-CHECKLIST.md`'s crawler-visibility findings: (1) Google AI Pro's BD student free offer ended 2026-03-11 but old coverage still ranks — add a "the offer ended, here's the alternative" section to the existing Google AI Pro page; (2) reseller ban-risk/legality is a real distinct search cluster, already answered by the chatbot (BACKLOG #24 done) but not in static HTML — add to `FAQPage.tsx`; (3) wide reseller price dispersion elsewhere (Midjourney BD ৳199-16,500/mo found across competitors) signals buyer confusion — add a short trust note to `/midjourney-bangladesh`; (4) SSC/HSC-level students are a distinct intent from the existing university-framed student guide — add a subsection; (5) bKash payment troubleshooting ("payment failed," "screenshot not received") is its own search intent — add to the existing bKash guide post; (6) Bangla-script title targeting — check `/bn` titles use literal Bangla script, not just transliteration | — (real content/copy work, all additions to existing pages per the "no mass page creation" rule — none need a new route) |
| 36 | A direct SERP competitor (OneBrain.app, found during the same 2026-08-07 research pass) runs near-duplicate-titled content to several of AIPS's own posts ("Claude Pro Price in BD: bKash Guide," "ChatGPT Price in BD 2026") and pitches a cheaper multi-model aggregator (৳199/mo). Not something to react to reflexively, but the owner should be aware of it as direct SERP competition for AIPS's exact target queries | — (owner awareness item, not a code task) |
| 37 | Honest, cited copy opportunity from the same research pass: real, sourced friction points (Bangladesh has ~2.9M credit-card holders per Bangladesh Bank; PayPal cannot be opened/funded from Bangladesh; OpenAI's ~89-country support list vs. Stripe's 164-country coverage can produce a payment-declined gap even with a working card) could strengthen the "How our pricing works" section and/or a new FAQ answer explaining *why* a reseller is needed — with real citations, not vague claims. Full sourcing in `RESEARCH-CACHE.md` | — (content work; needs the same copy-review care as any customer-facing claim, even though these are general market facts rather than AIPS-specific claims) |
